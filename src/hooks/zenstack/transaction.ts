...
export function useCheckTransaction<TError = DefaultError>(args: { operation: PolicyCrudKind; where?: { id?: string; stepperId?: string; hash?: string; gasUsed?: number }; }, options?: (Omit<UseQueryOptions<boolean, TError, boolean>, 'queryKey'> & ExtraQueryOptions)) {
    const { endpoint, fetch } = getHooksContext();
