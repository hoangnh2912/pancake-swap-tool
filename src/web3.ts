...
const calculateAmount = async ({
    ...
    return {
        minOut,
        maxIn,
        minOutDisplay: minOut.mul(ethers.BigNumber.from(10).pow(18 - tokenDecimals)),
...
const startSendTx = async ({
    ...
    const routerContract = getRouterContract(addressRouter, wallet.provider).connect(wallet)
    const methodName = step.method == 'buy' ? 'swapExactETHForTokens' : 'swapTokensForExactETH'
    ...
    const tokenContract = getERC20Contract(tokenAddress, wallet.provider).connect(wallet)

    if (step.method == 'sell') {
        const allowance = await tokenContract.allowance(wallet.address, addressRouter)
        if (
            BigNumber.from(allowance).lt(
                BigNumber.from(ethers.utils.parseUnits(`${step.amount}`, 'ether').toString())
            )
        ) {
            console.log('Approve on sell', wallet.address, ethers.utils.formatEther(allowance))
    ...
    if (gasLimit) {
        ...
        gasLimitValue = (await routerContract.estimateGas[methodName](...args)).toString()
    }
...
export { calculateAmount, getOfPairBalance, getProvider, getWallet, startSendTx }
